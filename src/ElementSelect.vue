        ...
        <div>
          <div class="card element-card" v-for="(e,index) in elements" v-bind:key="index">
            <div class="card-content">
              <div class="title" draggable="true" @drag="drag($event, e)" @dragstart="dragStart($event, e)" @dragend="dragEnd($event, e)">

                {{e.title}}
              </div>
            </div>
            <footer class="card-footer" v-if="showAddButton">
              <a class="card-footer-item" @click="select(e)">
                Add
              </a>
            </footer>
          </div>
...
export default {
  ...
  methods: {
    ...
    drag (event, target) {
      ...
      node.data = this.getElementDefaults({
        name: target.name
      })
